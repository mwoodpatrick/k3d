...
K3S_TAG := $(shell curl --silent --retry 3 "https://update.k3s.io/v1-release/channels/latest" | egrep -o '/v[^ ]+"' | sed -E 's/\/|\"//g' | sed -E 's/\+/\-/')

ifeq ($(K3S_TAG),)
$(warning K3S_TAG undefined: couldn't get latest k3s image tag!)
$(warning Output of curl: $(shell curl "https://update.k3s.io/v1-release/channels/latest"))
$(error exiting)
endif
